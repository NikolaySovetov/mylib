...
template<typename T>
list<T>::list(const mylib::allocator& a): alloc{a}, m_size{0} {    
#ifdef LIST_TEST
...
template<typename T>
list<T>::list(size_t sz, const T& t, const mylib::allocator& a): alloc{a}, m_size{0} {
#ifdef LIST_TEST
    tests::informator.PrintMess(list_set, 
        {"( type: ", typeid(T).name(), " ) created\n"}); 
#endif
...
template<typename T>
list<T>::list(const std::initializer_list<T>& init_list, const mylib::allocator& a)
: alloc{a}, m_size{0} {
#ifdef LIST_TEST
    tests::informator.PrintMess(list_set, 
...
template<typename T>
list<T>::list(const list& other): alloc {other.alloc}, m_size {0} {
#ifdef LIST_TEST
...
: alloc{std::move(other.alloc)}, base_node{other.base_node}, m_size{other.m_size} {
#ifdef LIST_TEST
...
void list<T>::emplace_back(Args&&... args) {
    insert_back(construct_node(std::forward<Args>(args)...));
}
...
// *** iterator ***
template<typename T>
list<T>::iterator::iterator(list<T>::base_node_type* ptr): p_node {ptr}  {
}     
...
T& list<T>::iterator::operator*() const {
    return static_cast<list<T>::node_type*>(p_node)->object;
}     
